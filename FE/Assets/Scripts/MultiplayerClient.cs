...
{
    public PropsController rewardController; 
    ...
    private void OnEvent(string name, byte[] data)
    {
        ...
        RewardRecognitionData reward = JsonUtility.FromJson<RewardRecognitionData>(Encode.GetString(data));
        if (reward.reward == "")
        {
            Debug.LogWarning("Empty Reward");
            return;
        }
        if(reward.playerName != "Chris Wild")
        {
            Debug.LogWarning("Player is not Chris");
            return;
        }
        rewardController.RecognitionDataDispatcher(reward); 
    }
